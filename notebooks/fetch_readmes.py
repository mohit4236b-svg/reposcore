...
df = pd.read_csv("data/repos_final.csv")
...
for i, row in df.iterrows():
    ...
    if (i + 1) % 100 == 0:
        try:
            pd.Series(readme_texts).to_csv(os.path.join(output_dir, "readme_texts_partial.csv"), index=False)
...
for attempt in range(5):
    try:
        df.to_csv(os.path.join(output_dir, "repos_with_readme.csv"), index=False)
        saved = True
        break
    except Exception as e:
