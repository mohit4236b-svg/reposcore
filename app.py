        ...
        try:
            ...
            if combined_score is not None:
                if combined_score >= 70:
                    ...
                    score_color = "red"
                    score_emoji = "🔴"
                score_card_html = f'<h2 class="section-header">⭐ Combined Score: {combined_score:.1f}/100 {score_emoji}</h2><div style="margin-bottom: 1rem;"><div style="background-color: #2d3548; border-radius: 6px; height: 20px; overflow: hidden;"><div style="background-color: {score_color}; width: {combined_score}%; height: 100%; border-radius: 6px; transition: width 0.3s ease;"></div></div><div style="font-size: 0.9em; color: #cbd5e0; margin-top: 0.5rem;">{combined_score:.1f}% — 50% ML Model + 50% Heuristic</div></div><div style="display: flex; gap: 1rem; margin-top: 1rem;"><div style="flex: 1; text-align: center; padding: 0.75rem; background: #2d3548; border-radius: 6px;"><div style="font-size: 0.85em; color: #94a3b8;">ML Model</div><div style="font-size: 1.5em; font-weight: 600; color: #e2e8f0;">{ml_score_pct:.1f}%</div></div><div style="flex: 1; text-align: center; padding: 0.75rem; background: #2d3548; border-radius: 6px;"><div style="font-size: 0.85em; color: #94a3b8;">Heuristic</div><div style="font-size: 1.5em; font-weight: 600; color: #e2e8f0;">{heuristic_score:.1f}/100</div></div><div style="flex: 1; text-align: center; padding: 0.75rem; background: #2d3548; border-radius: 6px;"><div style="font-size: 0.85em; color: #94a3b8;">Divergence</div><div style="font-size: 1.5em; font-weight: 600; color: {"#f87171" if abs(divergence) > 15 else "#e2e8f0"};">{divergence:+.1f}</div></div></div>'
