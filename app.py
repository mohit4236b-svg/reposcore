...
# Wire Streamlit secrets (e.g., NVIDIA_API_KEY) into os.environ so downstream modules can use os.getenv()
    ...
    <div style="flex: 1; text-align: center; padding: 0.75rem; background: #2d3548; border-radius: 6px;">
        ...
        <div style="font-size: 1.5em; font-weight: 600; color: {'#f87171' if abs(divergence) > 15 else '#e2e8f0'};">{divergence:+.1f}</div>
                ...
                if divergence > 15:
                    ...
                    render_caution(f"ML and Heuristic scores diverge by {divergence:.1f} points \u26A0\uFE0F treat this combined score with caution; review both scores individually in the **Why This Score?** tab.")
                ...
                with st.expander("Heuristic Score (RepoScorer)", expanded=True):
                    ...
                    try:
                        ...
                        if abs(delta) > 15:
                            ...
                            render_caution(f"ML and Heuristic scores diverge by {abs(delta):.1f} points \u26A0\uFE0F treat this combined score with caution; review both scores individually in the Why This Score? tab.")
